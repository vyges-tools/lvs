...
fn renamed_layout_matches_schematic() {
    ...
    assert!(
        r.matched,
        "renamed/reordered same chain should MATCH: {r:?}"
    );
...
fn analog_bandgap_renamed_layout_matches_schematic() {
    ...
    assert!(
        r.matched,
        "renamed/reordered analog bandgap should MATCH: {r:?}"
    );
    ...
    assert!(
        ...
        "a true match has no unbalanced classes"
    );
}
...
fn swapped_terminal_capacitor_still_matches() {
    ...
    assert!(
        r.matched,
        "swapped-order symmetric C must not break isomorphism: {r:?}"
    );
    assert!(
        r.verified,
